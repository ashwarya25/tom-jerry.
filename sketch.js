...
function setup()
 {
  ...
  ground=createSprite(600,displayHeight-160,1400,10);
  ...
  tom= createSprite(330, displayHeight-170, 50, 50);
  tom.addAnimation("running" , tomImg)
  tom.addAnimation("collided", tomImg2);
  tom.scale=1.2;
  tom.setCollider("rectangle",0,0,100,150);

  jerry=createSprite(displayWidth-500,displayHeight-230,20,20);
  jerry.addAnimation("teasing",jerryImg);
  jerry.scale=0.6;
  jerry.visible=false;
 
  spike=createSprite(70,displayHeight-270,20,20);
...
function draw()
{
  ...
  if ((touches.length > 0 || keyDown("space") &&tom.y>displayHeight-300)) 
  {
    tom.velocityY=-14;   
    jumpsound.play();
    touches = [];
...
function spawnObstacles() {
  if(frameCount % 100 === 0) {
    var obstacle = createSprite(1000,displayHeight-230,10,40);
    //obstacle.debug = true;
